...
<h4>Page Setting</h4>
<ul>
<li><a href="pages1.php?id=<?php echo $page_id;?>&name=<?php echo $pg_name;?>"><i class="fa fa-user"></i>Home</a></li>
...
<input type="hidden" value="<?php echo $pg_name;?>" name="page-name">
</div>
...
<span class="ad-mem-btn"><button class="cls"><a href="#" class="frn_req_rej" id="<?=$frnd_req['parent_id'] ?>">X</a></button><button><a href="#" class="frn_req_acc" id="<?= $grpres['pet_unique_id'] ?>">Invite Friend</a></button></span>
</div>
<?php
                            }
                        ?>
...
session_start();
$parent_id = $_SESSION["pet_unique_id"];
$grpqry=mysqli_query($conn,"SELECT * FROM pages WHERE user_id_fk='$parent_id' and status='1' LIMIT 10");
					WHILE($grpres=mysqli_fetch_assoc($grpqry)) {
						?>
