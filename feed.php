...
$userinfo=mysqli_fetch_assoc($userqry);
  ...
  //$display = "SELECT * FROM addfriend JOIN user_info ON addfriend.child_id = user_info.pet_unique_id JOIN post ON post.child_post_id = addfriend.child_id WHERE //addfriend.parent_id = '$parent_id' AND addfriend.status = '1' ORDER BY post.id DESC LIMIT 3";
  $display = "SELECT * FROM addfriend JOIN user_info ON addfriend.child_id = user_info.pet_unique_id JOIN post ON post.child_post_id = addfriend.child_id WHERE addfriend.parent_id = '$parent_id' AND addfriend.status >0 ORDER BY post.id DESC LIMIT 3";
...
WHILE($friendlist=mysqli_fetch_assoc($friendrun)) {
	$frn_nameqry=mysqli_query($conn,"SELECT * FROM user_info WHERE pet_unique_id='$friendlist[n]'");
           ...
           <span class="user-icn-img"><img src="images/pet-icon.png" alt="user image"><span class="fr-nm"><a href="profile-page1.php?id=<?= $row['pet_unique_id'] ?>" class="frn_profile"><?= $row['pet_name'] ?></a></span></span>
                        ...
                        <div class="fr-list fr-req">
                            <h2 class="fr-headnig"><span class="user-icn-img"><a href="#">
								...
								//$frnd_inf=mysqli_query($conn,"SELECT * FROM user_info WHERE pet_unique_id='$frnd_req[parent_id]'");
